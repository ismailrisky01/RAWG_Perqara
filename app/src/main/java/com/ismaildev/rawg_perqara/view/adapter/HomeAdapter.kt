...
class HomeAdapter : RecyclerView.Adapter<HomeAdapter.ViewHolder>() {
    ...
    private var listGame = emptyList<GameItem>()
    lateinit var context: Context
    fun setData(data: List<GameItem>, context: Context) {
        listGame = data
    ...
    class ViewHolder(private val binding: ItemGameBinding) : RecyclerView.ViewHolder(binding.root) {
        fun initData(gameItem: GameItem,context: Context) {
            binding.title.text = gameItem.name
            binding.rating.text = gameItem.rating.toString()
            binding.release.text = "Release date "+ gameItem.released
            Glide.with(context).load(gameItem.backgroundImage).into(binding.imageGame)
            val bundle = Bundle()
            bundle.putInt("id",gameItem.id!!)
